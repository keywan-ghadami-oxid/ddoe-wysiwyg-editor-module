...
return d=d.replace(/<img[^>]*src=\s*"([^"]+)"[^>]*data-filename=\s*"([^">]+)"[^>]*class=\s*"[^">]*dd-wysiwyg-media-image[^">]*"[^>]*>/gi,function(a,b,c){return a.replace(b,"[{$oViewConf->getMediaUrl()}]"+c)}),d=d.replace(/\[\{(([^\}\]]|\}[^\]]|[^\}]\])*)\}\]/gi,function(b,c,d){return a("<textarea />").html(b).text()})}})}(jQuery),function(a){/* Global define */
...
// Browser globals
a(window.jQuery)}(function(a){var b={wrapper:"row",columns:["col-md-12","col-md-6","col-md-4","col-md-3"]},c=function(c){var d=c.options.langInfo,e=c.options.callbacks,f=c.options.icons,g=a.summernote.ui,h=a.extend(b,c.options.grid||{}),i=this;c.memo("button.grid",function(){return g.buttonGroup([g.button({className:"dropdown-toggle",contents:'<i class="'+f.grid+'"/> <span class="note-icon-caret"></span>',tooltip:d.grid.tooltip,data:{toggle:"dropdown"}}),g.dropdown({className:"dropdown-menu dropdown-style text-grey-800 bg-white",contents:i.createDropdownContent(),callback:i.createGrid})]).render()}),this.createDropdownContent=function(){for(var a="",b=0;b<h.columns.length;b++)null!=h.columns[b]&&(a+=this.createDropdownElement(b));return a},this.createDropdownElement=function(a){var b=document.createElement("li"),c=document.createElement("a");return c.setAttribute("class","text-grey-800"),c.setAttribute("href","#"),c.setAttribute("data-index",a),c.innerText=d.grid.label+" #"+(a+1),b.appendChild(c),b.outerHTML},this.createGrid=function(b){b.find("li a").each(function(){a(this).click(function(){var b=a(this).data("index"),d=i.createGridNode(b);return e.onGridInsert?c.triggerEvent("grid.insert",d):c.invoke("editor.insertNode",d),!1})})},this.createGridNode=function(a){var b=document.createElement("div");b.className=h.wrapper;for(var c=0;c<=a;c++){var e=document.createElement("div"),f=document.createElement("p");e.className=h.columns[a],f.innerHTML=d.grid.label+" #"+(c+1),e.appendChild(f),b.appendChild(e)}return b}};a.extend(!0,a.summernote,{plugins:{gridPlugin:c},options:{grid:b,callbacks:{onGridInsert:null},icons:{grid:"fa fa-table"}},lang:{"en-US":{grid:{tooltip:"Columns",label:"Columns"}}}})}),/**
...
//[ 'fontname', [ 'fontname' ] ],
["fontsize",["fontsize"]],["color",["color"]],["layout",["ul","ol","paragraph"]],["height",["height"]],["table",["table"]],["insert",["link","video","hr"]],["misc",["codeview"]],["grid"]],dialogsInBody:!1,buttons:{ddmedia:"ddmedia"},disableDragAndDrop:!0});"disabled"===a(this).attr("disabled")&&a(this).summernote("disable");var e=d.data("summernote");e.invoke("codeview.activate"),e.invoke("codeview.deactivate")}});var b=a(".ddoe-wysiwyg-editor").first().closest("form");b.find('*[type="submit"]').first().on("click",function(){a(".ddoe-wysiwyg-editor > textarea",b).each(function(){var b=a(this).data("summernote");
